...
# BASIC FUNCTIONS TO USE FURTHER
...
class LogisticRegression(object):
    ...
    # initializing
    def __init__(self, dim, num_class = 10):
    ...
    # regularized_negative_log_likelihood realized manually (RegNLL)
...
def load_dataset(dataset):
    from util_self import load_dataset as load_d
    X_train, y_train, X_test, y_test = load_d('mnist')
...
def test(method='sgd', data='mnist', exp_decay=1, subset_size=0.4, greedy=1, shuffle=0, g_cnt=-1.,
    ...
    num_class = 10 if data in ['mnist'] else 3
    ...
    for itr in range(num_runs):
        ...
        if greedy == 1:
            ...
            if file_name != '':
                ...
                order, weights, _, _, ordering_time, similarity_time = util_self.get_orders_and_weights(
        ...
        for gamma in g_range:
            for b in b_range:
                ...
                if f_s < f_best:
                    ...
                    names = ['g', 'b','F_all_f', 'T_all', 'Acc_all', 'FT_all']
                    values = [g_f, b_f, f_runs_f,t_runs_f,acc_runs_f,ft_runs_f]

                    with open(f'{folder}_{method}_{subset_size}_{rand}_best_f_{metric}_w.txt', "a") as file:
                    ...
                    #np.save(f'{folder}_{method}_{subset_size}_{rand}_best_f_{metric}_w.npy', [g_f, b_f,
                             #x_runs_f, f_runs_f, t_runs_f, acc_runs_f, ft_runs_f])
                ...
                if acc_s > acc_best:
                    ...
                    names_a = ['g_a', 'b_a','F_all_f_a', 'T_all_a', 'Acc_all_a', 'FT_all_a']
                    values_a = [g_a, b_a, f_runs_a,t_runs_a,acc_runs_a,ft_runs_a]

                    with open(f'{folder}_{method}_{subset_size}_{rand}_best_acc_{metric}_w.txt', "a") as file:
                        for n,v in zip(names_a, values_a):
                            file.write(f'{n}  =  {v} \n ') 

                    #np.save(f'{folder}_{method}_{subset_size}_{rand}_best_acc_{metric}_w', [g_a, b_a,
                             #x_runs_a, f_runs_a, t_runs_a, acc_runs_a, ft_runs_a])
        ...
        with open(f'{folder}_{method}_{subset_size}_{rand}_best_f_{metric}_w.txt', "a") as file:
                        for n,v in zip(names, values):
                            file.write(f'{n}  =  {v} \n ') 
        '''
        np.save(f'{folder}_{method}_{subset_size}_{rand}_best_f_{metric}_w', [g_f, b_f,
                             x_runs_f, f_runs_f, t_runs_f, acc_runs_f, ft_runs_f])
        '''
